...
df_taxo <- read_tsv("rawdata/new_gtdbtk.bac120.tsv") %>% 
...
# concatenate files ------------------------------------------------------------
all_paths <- list.files(path = "rawdata/crisparcas/crispr_cas_loci/",
...
cas_phylum_genomes <- all_result %>% 
  ...
  scale_x_continuous(expand = c(0,0),
                     limits = c(0, 50),
                     breaks = seq(0, 50, 10)) +
  labs(x = "Genomes with CRISPR-Cas loci\n(%)",
       y = NULL) +
  theme_classic() +
  theme(text = element_text(size = 8),
        axis.text.y = element_text(size=9))

crispr_color <- c('#8dd3c7','#ffffb3','#bebada','#fb8072','#80b1d3','#fdb462',
...
pool_crispr <- crispr_abundance %>% 
  summarise(max = max(rel_abun),
            mean = mean(rel_abun),
            .by = Prediction) %>% 
  mutate(pool=if_else(max>7, FALSE, TRUE)) %>% 
  select(Prediction, pool)

phylum_factor <- c("UBP6", "Actinomycetota", "Bacteroidota","Verrucomicrobiota",
...
plot_crispr_abundance <- crispr_abundance %>% 
  ...
  summarise(sum_rel_abund = sum(rel_abun),
...
plot_crispr_abundance
 
df_taxo %>% 
   count(domain)


 df_crispr <- data.frame(domain = c("Archaea", "Bacteria"),
            crispr = c(9, 659),
