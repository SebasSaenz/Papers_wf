...
kingdom_number <- taxonomy %>% 
 ...
 barplots_total_ds <- all_result %>%
   ...
   scale_y_continuous(expand = c(0, 0),
                      limits = c(0, 100),
                      breaks = seq(0, 100, 20)) +
   labs(y = "Genomes with defence system (%)",
        x = NULL) +
   theme_classic()  +
...
# number of system families per genome
family_system <- all_result %>% #all genomes with DS
  ...
  scale_y_continuous(expand = c(0,0),
                     ...
                     breaks = seq(0, 35, 5),
                     oob = scales::squish) +
  labs(y = "Defence system families per genome",
       x = NULL) +
  theme_classic() +
  theme(text = element_text(size = 14),
        axis.title.y = element_text(size = 12),
...
genome_size_ds <- all_result %>% #all genomes with DS
 ...
 geom_point(position = position_jitter(),
  ...
  scale_fill_manual(values = colors) +
  facet_grid(rows = vars(kingdom)) +
  labs(y = "Number of defence systems",
       x = "Genome size (Mbp)") +
  theme_classic() +
  theme(text = element_text(size = 13),
        axis.text.x = element_text(size = 10),
...
system_list_filter <- all_result %>%
  ...
  mutate(total_genomes = case_when(kingdom == "Bacteria" ~ 2976,
                                   kingdom == "Archaea" ~ 62)) %>% 
...
rel_abund_system_kingdom <- all_result %>% #repalce status or kingdom
  filter(!grepl('_other', system)) %>%
...
taxon_pool <- rel_abund_system_kingdom %>%
  ...
  summarise(pool = mean(rel_abun) < 1.5,
            mean = mean(rel_abun),
            .groups ="drop")

col_list <- c('lightblue','#8dd3c7','#ffffb3','#fb8072','#bebada','#80b1d3','#fdb462',
...
ggsave(figure2, 
       file = "plots/figure2.png", width = 12, height = 8)
...
heat_map_df <- all_result %>%
  ...
  mutate(percent2 = case_when(percent >= 20 ~ 20,
                              T ~ percent),
...
ggplot(aes(x = system,
             ...
             fill = percent2)) +
  geom_tile() +
  geom_text(aes(label = round(percent)), color = "white", 
            size = 4, angle = 90) +
...
genus_ds_plot <- all_result %>%
  ...
  scale_x_continuous(expand = c(0, 0),
                     ...
                     breaks = seq(0, 36, 2)) +
  scale_fill_manual(values = colors) +
  labs(x = "Number of defence system",
       y = NULL) +
  theme_classic() +
...
genus_ds_fam_plot <- all_result %>%
  ...
  inner_join(list_system, by = "system") %>% 
  select(genome, system_family) %>% 
  unique() %>% mutate(genome = str_remove(genome, ".fasta_padloc.csv")) %>%
  count(genome) %>% 
  group_by(genome) %>% 
  summarise(total = mean(n), .groups = "drop") %>% 
  right_join(taxonomy, by = "genome") %>%
  ...
  scale_x_continuous(expand = c(0, 0),
                     ...
                     breaks = seq(0, 14, 2)) +
  scale_fill_manual(values = colors) +
  labs(x = "Number of defence system families",
       y = NULL) +
  theme_classic() +
  theme(text =element_text(size = 12),
        axis.text.y = element_markdown())
...
ggsave(figure4, file ="plots/figure4.png",
       width = 12, height = 6)
 ...
 all_result %>%
  ...
  right_join(taxonomy, by = "genome") %>%
  mutate(total = replace_na(total, 0)) %>% 
  select(kingdom, family, genus, total) %>% 
...
test <- number_system %>%  group_by(genus) %>%  count()
test2 <- number_system %>%  group_by(genus) %>%  summarise(sum_n = sum(n))
      
 x <- inner_join(test, test2, by = "genus") %>% 
   mutate(avg = sum_n/n) %>% 
   filter(n >=10)
 
 #correlation ds and dsf ---------
 genus_ds_plot <- all_result %>%
   select(genome, system.number, system) %>%
   filter(!grepl('_other', system)) %>% 
   unique() %>% 
